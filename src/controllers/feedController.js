...
const getAll = async (req, res) => {
    feedSchema.find(function (err, receitas) {
...
const getReceita = async (req, res) => {
    ...
    feedSchema.find({ "ingredientes": { $regex:req.params.ingrediente}},
        function (err, data) {
...
const alteraTitulo = async (req, res) => {
    ...
    const receitaFiltrada = await feedSchema.findOne({ "titulo": req.params.titulo })
    console.log(receitaFiltrada)
    receitaFiltrada.titulo = req.body.titulo
    receitaFiltrada.save()
    res.status(200).send({ "messagem": `${req.body.titulo}, alterada`, "dados": receitaFiltrada.titulo })
}
